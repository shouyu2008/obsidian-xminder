...
- **响应式布局** — 分屏或调整面板大小时自动重新适配视图；自定义布局引擎确保节点正确定位
...
│   ├── views/
│   │   ├── XMindView.ts         # FileView + mind-elixir 渲染
│   │   └── LayoutEngine.ts      # 自定义布局引擎
│   └── markdown/
│       ├── EmbedProcessor.ts    # 阅读模式 ![[]] / [[]] 后处理器
│       └── LivePreviewProcessor.ts  # 实时预览 CodeMirror 扩展
├── dist/                        # 构建产物
│   ├── main.js
│   ├── manifest.json
│   └── styles.css
